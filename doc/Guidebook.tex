...
%.lp
\item[\bb{Weight}]
The total weight of all items in your inventory, displayed if the
{\tt status} setting of the
{\it showweight\/}
option is enabled. The number after the slash is your carrying capacity.
%.lp
\item[\bb{Time}]
...
%.ed
\item[\tb{\^{}E}]
Write ``{\tt Elbereth}'' onto the ground.
Automatically uses a wield athame.
%.Ip
\item[\tb{f}]
...
Sit down.
\item[\tb{\#tip}]
Tip over a container to empty it of its contents.
%.lp
\item[\tb{\#turn}]
Turn undead.
%.lp
...
%.lp
\item[\ib{autounlock}]
Automatically apply unlocking tool when trying to open a locked door (default true).
%.lp
\item[\ib{boulder}]
...
%.lp
\item[\ib{invweight}]
Legacy alias for the
{\it showweight\/}
option with the {\tt items} setting.
%.lp
\item[\ib{legacy}]
...
%.lp
\item[\ib{showweight}]
Compound option controlling where weight information is displayed.
It takes a space separated list of {\tt +} or {\tt -} settings;
the recognized settings are {\tt inventory} (total carried weight and
carrying capacity in the full inventory heading), {\tt slots} (number
of used inventory slots in the full inventory heading), {\tt status}
(total carried weight and carrying capacity on the bottom status line)
and {\tt items} (weight of individual items in various listings).
For compatibility with the old boolean option, specifying
\ib{showweight} without any settings enables {\tt status}, while
{\tt !showweight} disables it; the other settings are not changed.
\ib{showweight:none}
disables all weight display, while
\ib{showweight:all}
enables all of them.
%.lp
\item[\ib{silent}]
