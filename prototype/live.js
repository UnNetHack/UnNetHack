...
export function installLive({scene,camera,controls,playerFactory,catFactory,monsterFactory,creatureFactory,wellTemplate,demoObjects,onDemo,onMode}) {
 ...
 const panel=document.createElement('section');panel.id='engine-panel';panel.hidden=true;panel.innerHTML='<div class="engine-toolbar"><b>UNNETHACK · LIVE ENGINE</b><button data-key="105">Inventory</button><button data-key="44">Pick up</button><button data-key="111">Open door</button><button data-key="113">Quaff</button><button data-key="83">Save & exit</button></div><div id="engine-status"></div><div id="engine-messages" role="log"></div><div id="engine-prompt"></div>';document.body.append(panel);
 ...
 function apply(frame){latest=frame;if(!active)return;
     ...
     if(cell.terrain!=='unknown'){
       ...
       if(!tile){tile=new THREE.Group();tile.position.set(x,0,z);tile.userData.type=cell.terrain;box(floorGeo,stone,tile,0,-.1,0);const fog=box(new THREE.PlaneGeometry(.98,.98),new THREE.MeshBasicMaterial({color:0x101a35,transparent:true,opacity:0,depthWrite:false}),tile,0,.012,0);fog.rotation.x=-Math.PI/2;tile.userData.fog=fog;
         ...
         if(cell.terrain==='wall'){
          ...
          if((cell.x*7+cell.z*11)%7===0){box(new THREE.CylinderGeometry(.045,.055,.3,8),wood,tile,0,.77,0);box(new THREE.CylinderGeometry(.09,.055,.1,8),iron,tile,0,.92,0);const fire=createFire(cell.x+cell.z);fire.position.y=.96;tile.add(fire);}
         }
   ...
   hero.target=new THREE.Vector3(frame.player.x-origin.x,0,frame.player.z-origin.z);
 ...
 function message(text){const p=document.createElement('div');p.textContent=text;$('#engine-messages').append(p);while($('#engine-messages').children.length>5)$('#engine-messages').firstChild.remove();$('#message').textContent=text;}
 ...
 async function reply(value){if(!pending)return;const req=pending;pending=null;lines=[];menu=null;dialog.close();$('#engine-prompt').textContent='Engine is resolving your action…';try{await post('/engine/input',{id:req.id,value});}catch(e){message(e.message);$('#engine-prompt').textContent='Input was not accepted. Reconnect Live mode to refresh the prompt.';}}
 function prompt(){if(!active||!pending)return;$('#engine-prompt').textContent=pending.prompt;if(pending.kind==='command')return;
   dialog.replaceChildren();const title=document.createElement('h2');title.textContent=pending.prompt||'UnNetHack';dialog.append(title);
