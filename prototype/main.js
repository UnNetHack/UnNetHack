...
function knight(){const g=new THREE.Group();const body=new THREE.Group();g.add(body);const armor=mat('#58727b',{map:metalSurface.map,bumpMap:metalSurface.bump,bumpScale:.1,metalness:.82,roughness:.26}),armorLight=mat('#a7c2c5',{map:metalSurface.map,bumpMap:metalSurface.bump,bumpScale:.08,metalness:.76,roughness:.22}),boots=mat('#20292c',{roughness:.72}),leather=mat('#533c2f',{roughness:.9}),visor=mat('#101c20',{metalness:.5,roughness:.32}),accent=mat('#c8a45a',{metalness:.78,roughness:.26});const legs=[];for(const x of [-.14,.14]){const pivot=new THREE.Group();pivot.position.set(x,.49,0);body.add(pivot);roundedBox(.19,.4,.2,armor,pivot,0,-.14,0,.05);roundedBox(.19,.22,.2,leather,pivot,0,-.30,0,.035);
 ...
 return {g,body,legs,arm,elbow,wrist,head,shieldArm,shieldElbow,cape,plume,setWeapon,setHelmet};}
...
const gallery=new THREE.Group();const galleryActors=[];const galleryNames=['gnome','orc','dwarf','guard','shopkeeper','dog','unicorn','dragon','grid bug'];const galleryColumns=[-3.3,0,3.3];const galleryRows=[-1.8,0,1.8];for(const [i,name] of galleryNames.entries()){const actor=createCreature({name,kind:'monster'});actor.g.position.set(galleryColumns[i%3],0,galleryRows[Math.floor(i/3)]);actor.g.add(galleryLabel(name));gallery.add(actor.g);galleryActors.push({actor,phase:i*.73});}gallery.visible=false;scene.add(gallery);
...
renderer.setAnimationLoop(ms=>{const t=ms/1000,dt=Math.min(t-lastTime,.05);lastTime=t;const mix=1-Math.exp(-dt*14);
 ...
 const swing=Math.max(0,1-(t-attackTime)/.35);player.arm.rotation.x=-Math.sin(swing*Math.PI)*1.9;
