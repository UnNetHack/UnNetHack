...
static boolean maybe_break_wand_trap(int, int, int);
        ...
        case 1:
            if (!ublindf) {
                ...
                what = (ublindf->otyp == LENSES) ? "lenses" :
                       (obj->otyp == ublindf->otyp) ? "other towel" : "blindfold";
...
its_dead(int rx, int ry, int *resp, struct obj *tobj)
{
    ...
    boolean more_corpses = (corpse && nxtobj(corpse, CORPSE, TRUE));
    ...
    if ((mtmp = m_at(rx, ry)) != 0) {
        ...
        if (mtmp->mundetected) {
            ...
            case M_AP_OBJECT:
                ...
                use_plural = (is_boots(odummy) || is_gloves(odummy) || odummy->otyp == LENSES);
                break;
...
static const char alt_whistle_str[] = "produce a %s, sharp vibration.";

static void
use_whistle(struct obj *obj)
{
    if (!can_blow(&youmonst)) {
        You("are incapable of using this whistle.");
    } else if (Underwater) {
...
use_leash(struct obj *obj)
{
    ...
    if (u.uswallow) {
        ...
        You_cant((!obj->leashmon ? "leash %s from inside." :
                 (obj->leashmon == (int) u.ustuck->m_id) ?
                  "unleash %s from inside." : "unleash anything from inside %s."),
...
got_target:
    ...
    if (!spotmon && !glyph_is_invisible(levl[cc.x][cc.y].glyph)) {
        ...
        if (mtmp->mleashed) {
            ...
            pline("The leash won't fit onto %s%s.", spotmon ? "your " : "", l_monnam(mtmp));
        } else {
...
#define SEENMON (MONSEEN_NORMAL | MONSEEN_SEEINVIS | MONSEEN_INFRAVIS)
    how_seen = vis ? how_mon_is_seen(mtmp) : 0;
    ...
    if (mtmp->msleeping) {
        ...
        if (vis) {
            pline("%s is too far away to see %sself in the dark.", Monnam(mtmp), mhim(mtmp));
        }
        ...
        mtmp->mconf = 1;
               ...
               (mlet == S_NYMPH || mtmp->data == &mons[PM_SUCCUBUS] || mtmp->data == &mons[PM_INCUBUS])) {
        if (vis) {
            char buf[BUFSZ]; /* "She" or "He" */
            pline("%s admires %sself in your %s.", Monnam(mtmp), mhim(mtmp), mirror);
        ...
        if (mtmp->minvis && !See_invisible) {
            ...
            pline("%s doesn't seem to notice %s reflection.", Monnam(mtmp), mhis(mtmp));
        } else {
...
use_lamp(struct obj *obj)
{
    ...
    if (Underwater) {
        pline(!Is_candle(obj) ? "This is not a diving lamp" : "Sorry, fire and water don't mix.");
        return;
    }
...
is_valid_jump_pos(coordxy x, coordxy y, int magic, boolean showmsg)
{
    if (!magic && !(HJumping & ~INTRINSIC) && !EJumping && distu(x, y) != 5) {
        ...
        if (diag == jDiag &&
            ...
            (traj == jDiag || ((traj & jHorz) != 0) == (lev->horizontal != 0))) {
            if (showmsg) {
        ...
        if (u.utrap) {
            ...
            case TT_INFLOOR:
                You("strain your %s, but you're still %s.",
                    ...
                    (u.utraptype == TT_INFLOOR) ? "stuck in the floor" : "attached to the buried ball");
...
#define TimedTrouble(P) (((P) && !((P) & ~TIMEOUT)) ? ((P) & TIMEOUT) : 0L)
    ...
    for (idx = 0; idx < A_MAX; idx++) {
        ...
        if (val_limit > ABASE(idx) + attr_point) {
            val_limit = ABASE(idx) + attr_point;
        }
 ...
 */
void
fig_transform(anything *arg, long int timeout)
{
    ...
    if (mtmp) {
        ...
        if ((mtmp->minvis && !See_invisible) ||
             (mtmp->data->mlet == S_MIMIC && M_AP_TYPE(mtmp) != M_AP_NOTHING)) {
            suppress_see = TRUE;
        }

        if (mtmp->mundetected) {
            if (hides_under(mtmp->data) && mshelter) {
                Sprintf(and_vanish, " and %s under %s", locomotion(mtmp->data, "crawl"), doname(mshelter));
        ...
        case OBJ_MINVENT:
            if (cansee_spot && !silent && !suppress_see) {
                ...
                You_see("%s %s out of %s%s!",
                        monnambuf, locomotion(mtmp->data, "drop"), carriedby, and_vanish);
            }
            break;
#if 0
        case OBJ_MIGRATING:
...
use_trap(struct obj *otmp)
{
    ...
    if (nohands(youmonst.data)) {
        ...
        what = (levl[u.ux][u.uy].typ == AIR) ? "in midair" :
               (levl[u.ux][u.uy].typ == CLOUD) ? "in a cloud" : "in this place"; /* Air/Water Plane catch-all */
    }
    if (what) {
    ...
    if (u.uswallow && attack(u.ustuck)) {
        ...
        if (otmp) {
            ...
            if (gotit) {
                ...
                case 2:
                    /* to floor near you */
                    You("yank %s to the %s!", yname(otmp), surface(u.ux, u.uy));
...
use_pole(struct obj *obj)
{
    ...
    coord cc;
    struct monst *mtmp;
    struct monst *hitm = polearm.hitmon;
    ...
    if ((mtmp = m_at(bhitpos.x, bhitpos.y))) {
        ...
        if (t && t->ttyp == STATUE_TRAP && activate_statue_trap(t, t->tx, t->ty, FALSE)) {
...
use_grapple (struct obj *obj)
{
    ...
    case 2: /* Monster */
        ...
        notonhead = (bhitpos.x != mtmp->mx || bhitpos.y != mtmp->my);
        boolean save_confirm = flags.confirm;
...
broken_wand_explode(struct obj *obj, int dmg, int expltype)
{
    ...
    makeknown_msg(obj->otyp, TRUE); /* explode describes the effect */
    discard_broken_wand();
}

/* Try to create a trap at the hero's position from a broken wand.
   Returns TRUE if a trap was created. */
static boolean
maybe_break_wand_trap(int typ, int spe, int traptype)
{
    struct trap *ttmp;

    if ((spe > 2) && rn2(spe - 2) && !u.uswallow &&
        ...
        if (ttmp) {
            ...
            ttmp->tseen = 1;
            newsym(u.ux, u.uy);
            pline("%s appears beneath you!", An(trapname(traptype, FALSE)));
            makeknown_msg(typ, TRUE); /* explode describes the effect */
            if (*in_rooms(u.ux, u.uy, SHOPBASE)) {
...
static void
discard_broken_wand_with_msg(const char *msg)
{
    pline("%s", msg);
    discard_broken_wand();
}
...
#define BY_OBJECT ((struct monst *) 0)
    ...
    coordxy x, y;
    struct monst *mon;
    int dmg, damage, wand_spe, wand_otyp;
    ...
    wand_spe = obj->spe;
    wand_otyp = obj->otyp;
    int break_wand_trap_type = NO_TRAP;

    switch (obj->otyp) {
    case WAN_OPENING:
        /* make trap door if you broke a wand of opening */
        if (Can_dig_down(&u.uz)) {
            maybe_break_wand_trap(wand_otyp, wand_spe, TRAPDOOR);
            discard_broken_wand();
        } else {
            discard_broken_wand_with_msg(nothing_else_happens);
        }
        return ECMD_TIME;

    case WAN_NOTHING:
        if (obj->dknown && objects[obj->otyp].oc_name_known) {
            discard_broken_wand_with_msg("Suddenly, and without warning, nothing happens.");
            return ECMD_TIME;
        } else {
            discard_broken_wand_with_msg(nothing_else_happens);
        }
    ...
    case WAN_WISHING:
        if (obj->dknown && objects[obj->otyp].oc_name_known) {
            ...
            discard_broken_wand_with_msg("You really wish you hadn't done that.");
        } else {
            discard_broken_wand_with_msg(nothing_else_happens);
        }
        return ECMD_TIME;
    ...
    case WAN_LIGHTNING:
        ...
        maybe_break_wand_trap(wand_otyp, wand_spe, MAGIC_TRAP);
        return ECMD_TIME;

    case WAN_FIRE:
        broken_wand_explode(obj, dmg * 2, EXPL_FIERY);
        maybe_break_wand_trap(wand_otyp, wand_spe, FIRE_TRAP);
        return ECMD_TIME;

    case WAN_COLD:
        broken_wand_explode(obj, dmg * 2, EXPL_FROSTY);
        maybe_break_wand_trap(wand_otyp, wand_spe, ICE_TRAP);
        return ECMD_TIME;
    ...
    case WAN_TELEPORTATION:
        if (!noteleport_level(&youmonst)) {
            break_wand_trap_type = TELEP_TRAP;
        }
        affects_objects = TRUE;
        break;

    case WAN_POLYMORPH:
        break_wand_trap_type = POLY_TRAP;
        affects_objects = TRUE;
        break;

    case WAN_SLEEP:
        break_wand_trap_type = SLP_GAS_TRAP;
        break;

    case WAN_CANCELLATION:
        break_wand_trap_type = ANTI_MAGIC;
        affects_objects = TRUE;
        break;
    ...
    if (break_wand_trap_type != NO_TRAP) {
        maybe_break_wand_trap(wand_otyp, wand_spe, break_wand_trap_type);
    }
    ...
    for (i = 0; i <= 8; i++) {
        ...
        if (obj->otyp == WAN_DIGGING) {
            if (dig_check(BY_OBJECT, FALSE, x, y)) {
                 ...
                 */
                schar typ = fillholetyp(x, y, FALSE);
                if (typ != ROOM) {
                    ...
                    digactualhole(x, y, BY_OBJECT,
                                (rn2(obj->spe) < 3 ||
                                 (!Can_dig_down(&u.uz) && !levl[x][y].candig)) ? PIT : HOLE);
                }
            }
            continue;
...
unfixable_trouble_count(boolean is_horn)
{
     ...
     */
    if (Sick && (!is_horn || (Sick & ~TIMEOUT))) {
        unfixable_trbl++;
    }
    if (Stunned && (!is_horn || (HStun & ~TIMEOUT))) {
        unfixable_trbl++;
    }
    if (Confusion && (!is_horn || (HConfusion & ~TIMEOUT))) {
        unfixable_trbl++;
    }
    if (Hallucination && (!is_horn || (HHallucination & ~TIMEOUT))) {
        unfixable_trbl++;
    }
    if (Vomiting && (!is_horn || (Vomiting & ~TIMEOUT))) {
        unfixable_trbl++;
    }
    if (Deaf && (!is_horn || (HDeaf & ~TIMEOUT))) {
        unfixable_trbl++;
    }
