...
dig_check(struct monst *madeby, boolean verbose, coordxy x, coordxy y)
{
    ...
    if (On_stairs(x, y)) {
        ...
        return FALSE;
            ...
            (madeby != BY_OBJECT || (altarmask_at(x, y) & AM_SANCTUM) != 0)) {
        if (verbose) {
...
dig(void)
{
    ...
    if (digging.down) {
        ...
        if (digging.effort > 250 || (ttmp && ttmp->ttyp == HOLE)) {
        ...
        if (digging.effort <= 50 ||
            ...
            (void) memset((genericptr_t) &digging, 0,
                          sizeof digging);
            return 0;
    ...
    boolean fill_if_any) /**< force filling if it exists at all */
{
    ...
    if ((lava_cnt > moat_cnt + pool_cnt && rn2(lava_cnt + 1)) ||
        ...
        return POOL;
    } else if ((swamp_cnt > 0 && rn2(swamp_cnt + 1)) || (swamp_cnt && fill_if_any)) {
        return BOG;
    } else {
        return ROOM;
    }
            ...
            cant_reach_floor(u.ux, u.uy, FALSE, TRUE);
               ...
               /* can only dig down with an axe when doing so will
                  ...
                  (!trap || (trap->ttyp != LANDMINE &&
                             trap->ttyp != BEAR_TRAP))) {
        pline("%s merely scratches the %s.", Yobjnam2(obj, (char *) 0), surface(u.ux, u.uy));
        u_wipe_engr(3);
    } else {
