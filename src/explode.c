...
int expltype;
{
    ...
    boolean do_hallu = FALSE;
    boolean inside_engulfer;
    boolean grabbed;
    boolean grabbing;
    coord grabxy;
    ...
    if (olet == WAND_CLASS) { /* retributive strike */
        ...
        if (type < 0) {
            ...
            if (objects[type].oc_dir == RAY && type != WAN_DIGGING && type != WAN_SLEEP) {
        ...
        for (i=0; i<3; i++) {
            for (j=0; j<3; j++) {
                ...
                if (do_hallu) {
                    ...
                    do {
                        Sprintf(hallu_buf, "%s explosion", s_suffix(rndmonnam()));
