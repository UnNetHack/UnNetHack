                ...
                if (is_watch(mtmp->data) &&
                    ...
                    if (!Deaf) {
                        ...
                        pline("%s earnestly %s %s %s!",
                              ...
                              mhis(mtmp),
                              nolimbs(mtmp->data) ? mbodypart(mtmp, HEAD) : makeplural(mbodypart(mtmp, ARM)));
                    }
                    break;
                }
            }
    ...
    if (obj->otyp == LONG_SWORD && obj->quan == 1L
        ...
        if (u.ualign.type != A_LAWFUL) {
            ...
            livelog_printf(LL_ARTIFACT,
                           "was denied Excalibur! The Lady of the Lake has deemed %s unworthy",
                           uhim());
        } else {
        ...
        if (mvitals[PM_SEWER_RAT].mvflags & G_GONE) {
            ...
            if (mtmp) {
                pline("Eek!  There's %s in the sink!",
                      (Blind || !canspotmon(mtmp)) ? "something squirmy" : a_monnam(mtmp));
            }
        }
        break;
