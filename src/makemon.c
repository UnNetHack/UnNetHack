    ...
    switch (ptr->mlet) {
        ...
        case S_ANGEL:
            if (mm == PM_ALEAX) {
                ...
                /* mimic your armor */
                if (uarm) {
                    m_inityour(mtmp, uarm);
                }
                if (uarmc) {
                    m_inityour(mtmp, uarmc);
                }
                if (uarmh) {
                    m_inityour(mtmp, uarmh);
                }
                if (uarms) {
                    m_inityour(mtmp, uarms);
                }
                if (uarmg) {
                    m_inityour(mtmp, uarmg);
                }
                if (uarmf) {
                    m_inityour(mtmp, uarmf);
                }
                if (uarmu) {
                    m_inityour(mtmp, uarmu);
                }
 ...
 */
void
mkmonmoney(struct monst *mtmp, long int amount)
{
...
static boolean
makemon_rnd_goodpos(struct monst *mon, unsigned int gpflags, coord *cc)
{
    int tryct = 0;
    int nx,ny;
    boolean good;
...
void
newmonhp(struct monst *mtmp, int mndx)
{
...
#endif
            if (prohibited_by_generation_flags(ptr)) {
                continue;
            }
            ct = (int)(ptr->geno & G_FREQ) + align_shift(ptr);
            /* Boost Sheol-Only(tm) monster generation in Sheol. */
            if (Insheol && (ptr->geno & G_SHEOL)) {
                ct *= 2;
            }
            if (ct < 0 || ct > 127) {
...
int
adj_lev(struct permonst *ptr) /* adjust strength of monsters based on u.uz and u.ulevel */
...
void
set_mimic_sym(struct monst *mtmp) /* KAA, modified by ERS */
