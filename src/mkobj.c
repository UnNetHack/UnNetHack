...
struct obj *
mksobj_migr_to_species(int otyp, unsigned int mflags2, boolean init, boolean artif)
{
    struct obj *otmp;
...
rndmonnum(void)
{
    ...
    do {
        ...
        ptr = &mons[i];
        count++;
    } while (((ptr->geno & excludeflags) || prohibited_by_generation_flags(ptr)) && count < 10000);
...
nextoid(struct obj *oldobj, struct obj *newobj)
{
    ...
    unsigned oid = flags.ident - 1; /* loop increment will reverse -1 */
    ...
    *otmp = zeroobj;
    ...
    if (init) {
        ...
        default:
            ...
            panic("mksobj tried to make type %d, class %d.", (int) otmp->otyp, (int) objects[otmp->otyp].oc_class);
            /*NOTREACHED*/
        }
    }
...
obj_sanity_check(void)
{
    ...
    if (current_wand) {
        insane_object(current_wand, ofmt3, "current_wand sanity", (struct monst *) 0);
    }
}
    ...
    for (obj = objlist; obj; obj = obj->nobj) {
        ...
        if (Has_contents(obj)) {
            if (wheretype == OBJ_ONBILL) {
                ...
                insane_object(obj, "%s obj contains something! %s %s: %s", mesg, (struct monst *) 0);
            }
...
insane_object(struct obj *obj, const char *fmt, const char *mesg, struct monst *mon)
{
    ...
    if (mon || (strstri(mesg, "minvent") && !strstri(mesg, "contained"))) {
        ...
        impossible(altfmt, mesg, fmt_ptr((genericptr_t) obj), where_name(obj), objnm,
 ...
 */
struct obj *
init_dummyobj(struct obj *obj, short int otyp, long int oquan)
{
     if (obj) {
         *obj = zeroobj;
         obj->otyp = otyp;
         ...
         obj->known = (obj->oclass == AMULET_CLASS) ? obj->known
...
check_contained(struct obj *container, const char *mesg)
{
    ...
    for (obj = container->cobj; obj; obj = obj->nobj) {
        ...
        if (Has_contents(obj)) {
            ...
            copynchars(eos(nestedmesg), mesg, (int)sizeof(nestedmesg) - (int)strlen(nestedmesg)-1);
    ...
    if (n == 1 && (carried(obj) || (owornmask & (W_BALL | W_CHAIN)) != 0L
        ...
        if (owornmask & W_ARMOR) {
            ...
            if (mcarried(obj) && (owornmask & (W_SWAPWEP | W_QUIVER)) != 0L) {
                what = (owornmask & W_SWAPWEP) != 0L ? "monst alt weapon?" : "monst quiver?";
            /* hero can quiver gold but not wield it (hence not alt-wield
               it either); also catches monster wielding gold */
            } else if (obj->oclass == COIN_CLASS && (owornmask & (W_WEP | W_SWAPWEP)) != 0L) {
...
obj_absorb(struct obj **obj1, struct obj **obj2)
{
    ...
    if (obj1 && obj2) {
        ...
        if (otmp1 && otmp2 && otmp1 != otmp2) {
            ...
            agetmp = (((moves - otmp1->age) * o1wt + (moves - otmp2->age) * o2wt) / (o1wt + o2wt));
