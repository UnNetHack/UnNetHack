...
somexyspace(struct mkroom *croom, coord *pos, int flags)
{
    ...
    do {
        ...
        if ((flags & 1) && (IS_POOL(levl[pos->x][pos->y].typ) || IS_FURNITURE(levl[pos->x][pos->y].typ))) {
            isok = FALSE;
        }
        if ((flags & 2) && ((levl[pos->x][pos->y].typ != CORR) || (levl[pos->x][pos->y].typ != ROOM))) {
            isok = FALSE;
        }
        ...
        if ((flags & 8) && bydoor(pos->x, pos->y)) {
            isok = FALSE;
              ...
              !SPACE_POS(levl[pos->x][pos->y].typ) ||
