...
monmulti(struct monst *mtmp, struct obj *otmp, struct obj *mwep)
{
    ...
    if ((otmp->quan > 1L) && /* no point checking if there's only 1 */
        ...
        if (mwep && mwep->otyp == ELVEN_BOW && ammo_and_launcher(otmp, mwep) && !mwep->cursed) {
            multishot++;
        }
        ...
        /* racial bonus */
        if ((is_elf(mtmp->data) && otmp->otyp == ELVEN_ARROW && mwep->otyp == ELVEN_BOW) ||
            (is_orc(mtmp->data) && otmp->otyp == ORCISH_ARROW && mwep->otyp == ORCISH_BOW) ||
            (is_gnome(mtmp->data) && otmp->otyp == CROSSBOW_BOLT && mwep->otyp == CROSSBOW)) {
            multishot++;
        }
    }
    ...
    boolean verbose)    /**< give message(s) even when you can't see what happened */
{
    ...
    if (tmp < rnd(20)) {
        ...
        if (objects[otmp->otyp].oc_material == SILVER && mon_hates_silver(mtmp)) {
            boolean flesh = (!noncorporeal(mtmp->data) && !amorphous(mtmp->data));
            ...
            if (vis) {
                ...
                if (flesh) {
                    ...
                    m_name = strcat(s_suffix(m_name), " flesh"); pline_The("silver sears %s!", m_name);
                }
            } else if (verbose && !target) {
        ...
        if (!DEADMONSTER(mtmp)) { /* might already be dead (if petrified) */
            ...
            if (DEADMONSTER(mtmp)) {
                if (vis || (verbose && !target)) {
                    boolean is_undead = (nonliving(mtmp->data) || is_vampshifter(mtmp) || !canspotmon(mtmp));
                    pline("%s is %s!", Monnam(mtmp), is_undead ?  "destroyed" : "killed");
                }
            ...
            if (hitu &&
               can_blnd((struct monst*)0, &youmonst,
                ...
                if (singleobj->otyp == CREAM_PIE) {
                    if (!Blind) {
                        pline("Yecch!  You've been %s.", piday() ? "pied" : "creamed");
                    } else {
        ...
        if (!range || /* reached end of path */
            ...
            if (singleobj) {
                ...
                if (m_shot.n > 1 &&
                        ...
                        (cansee(bhitpos.x, bhitpos.y) || (archer && canseemon(archer))))
    ...
    if (!ispole && m_lined_up(mtarg, mtmp)) {
        ...
        if (!mtarg->mflee || !rn2(chance)) {
            if (ammo_and_launcher(otmp, mwep) &&
                dist2(mtmp->mx, mtmp->my, mtarg->mx, mtarg->my) > PET_MISSILE_RANGE2) {
...

{
    ...
    if (Upolyd && rn2(25) &&
        (u.uundetected || (U_AP_TYPE != M_AP_NOTHING && U_AP_TYPE != M_AP_MONSTER))) {
        return FALSE;
    }
