...
precheck(struct monst *mon, struct obj *obj)
{
    ...
    if (obj->oclass == WAND_CLASS && obj->cursed && !rn2(WAND_BACKFIRE_CHANCE)) {
        ...
        if (vis) {
            pline("%s %s %s, which suddenly explodes!",
...
m_use_healing(struct monst *mtmp)
{
    ...
    if ((obj = m_carrying(mtmp, POT_EXTRA_HEALING))) {
        m.defensive = obj;
        m.has_defense = MUSE_POT_EXTRA_HEALING;
        return TRUE;
    }
    if ((obj = m_carrying(mtmp, POT_HEALING))) {
        m.defensive = obj;
        m.has_defense = MUSE_POT_HEALING;
        return TRUE;
    }
    if (is_vampire(mtmp->data) &&
        (obj = m_carrying(mtmp, POT_VAMPIRE_BLOOD))) {
        m.defensive = obj;
...
find_defensive(struct monst *mtmp)
{
    ...
    if (stuck || immobile) {
        ...
        boolean diag_ok = !NODIAG(monsndx(mtmp->data));
...
#define m_flee(m)   if (fleetim && !m->iswiz) \
    ...
    case MUSE_UNICORN_HORN:
        ...
        if (!mtmp->mcansee) {
            ...
            warning("No need for unicorn horn?");
        }
        return 2;
    case MUSE_BUGLE:
        if (vismon) {
...
find_misc(struct monst *mtmp)
{
    ...
    int pmidx = monsndx(mdat);
    if (!stuck && !immobile && !mtmp->cham && monstr[pmidx] < 6) {
...
#define nomore(x) if (m.has_misc==x) continue;
    ...
    for (obj=mtmp->minvent; obj; obj=obj->nobj) {
        ...
        if (obj->oartifact == ART_ITLACHIAYAQUE &&
             obj->age <= monstermoves &&
             !mtmp->mpeaceful &&
             !mtmp->mblinded &&
             m_canseeu(mtmp)) {
            m.misc = obj;
            m.has_misc = MUSE_ITLACHIAYAQUE;
        }
    }
...
skipmsg:
    ...
    case MUSE_BULLWHIP:
        ...
        if (uwep && !rn2(5)) {
            ...
            if (!obj || !canletgo(obj, "") || (u.twoweap && canletgo(uswapwep, "") && rn2(2))) {
                obj = uswapwep;
            }
            if (!obj) {
    ...
    case MUSE_ITLACHIAYAQUE:
        ...
        otmp->age = monstermoves + (mtmp->data == &mons[PM_SCHLIEMANN] ? rn1(20,10) : rnz(100));
        if (cansee(mtmp->mx, mtmp->my)) {
            pline("%s brandishes Itlachiayaque!", Monnam(mtmp));
            pline("A noxious cloud pours forth!");
        } else if (!Blind) {
            pline("A noxious cloud envelops you!");
        } else if (!breathless(youmonst.data)) {
            pline("Noxious air seems to surround you!");
        }
        create_gas_cloud(u.ux, u.uy, 15 + 10 * bcsign(otmp), 8 + 4 * bcsign(otmp), rn1(3, 4));
        return 2;
    ...
    default:
        warning("%s wanted to perform action %d?", Monnam(mtmp), m.has_misc);
        break;
    }
    return 0;
}
...
munstone(struct monst *mon, boolean by_you)
{
    ...
    boolean tinok = mcould_eat_tin(mon);
...
mcould_eat_tin(struct monst *mon)
{
    ...
    boolean welded_wep = mwep && mwelded(mwep);
    ...
    for (obj = mon->minvent; obj; obj = obj->nobj) {
        ...
        if (obj->otyp == TIN_OPENER ||
            (obj->oclass == WEAPON_CLASS &&
             (objects[obj->otyp].oc_skill == P_DAGGER || objects[obj->otyp].oc_skill == P_KNIFE))) {
            return TRUE;
        }
    }
    return FALSE;
...
munslime(struct monst *mon, boolean by_you)
{
    ...
    if (!mon->mcan && !mon->mspec_used && attacktype_fordmg(mptr, AT_BREA, AD_FIRE)) {
    ...
    if (!is_animal(mptr) && !mindless(mptr)) {
        ...
        if (((t = t_at(mon->mx, mon->my)) == 0 || t->ttyp != FIRE_TRAP) &&
            ...
            for (idx = 0; idx < nxy; ++idx) {
                ...
                if ((t = t_at(xy[0][idx], xy[1][idx])) != 0 && t->ttyp == FIRE_TRAP) {
                    break;
                }
            }
        }
...
{               /* [by_you not honored if 'mon' triggers fire trap]. */
    ...
    if (trap) {
        ...
        if (mon->mx == trap->tx && mon->my == trap->ty) {
            ...
            newsym(mon->mx, mon->my);
            if (vis) {
                pline("%s %ss %s %s fire trap!", Mnam,
                      locomotion(mon->data, "move"),
...
green_mon(struct monst *mon)
{
    ...
    default:
        if (is_elf(ptr) && !is_prince(ptr) && !is_lord(ptr) && ptr != &mons[PM_GREY_ELF]) {
            return TRUE;
        }
        break;
    }
