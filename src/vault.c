...
boolean forceshow;
{
    ...
    boolean silently = program_state.stopprint ? TRUE : FALSE;
...
invault()
{
    ...
    if (++u.uinvault % VAULT_GUARD_TIME == 0 && !guard) {
        ...
        if (Deaf) {
            pline("%s doesn't %srecognize you.", noit_Monnam(guard), (Blind) ? "" : "appear to ");
        } else {
        ...
        if (!umoney && !hidden_gold()) {
            if (Deaf) {
                pline("%s stomps%s.", noit_Monnam(guard), (Blind) ? "" : " and beckons");
            } else {
            ...
            if (!umoney) {
                if (Deaf) {
                    if (!Blind) {
                        pline("%s glares at you%s.", noit_Monnam(guard), invent ? "r stuff" : "");
                    }
                } else {
    ...
    if (egrd->fcend > 1) {
        ...
        if (u_carry_gold &&
            ...
            if (egrd->warncnt < 6) {
                ...
                if (Deaf) {
                    if (!Blind) {
                        pline("%s holds out %s palm demandingly!", noit_Monnam(grd), noit_mhis(grd));
                    }
                } else {
